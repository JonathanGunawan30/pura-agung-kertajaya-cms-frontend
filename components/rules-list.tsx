...
export function RulesList() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const result = await rulesApi.get(entityType)
            setData(result)
        } catch (error) {
    ...
    if (showForm) {
        return <RulesForm initialData={data} entityType={entityType} onClose={handleFormClose} />
    }
            ...
            <div className="rounded-xl border bg-card text-card-foreground shadow-sm overflow-hidden">
                <div className="p-5 bg-background/50 backdrop-blur-sm flex justify-between items-center">
                    <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span className="text-sm font-medium">Tata Tertib & Peraturan <span className="capitalize font-bold text-foreground">{entityType}</span></span>
                    </div>
                    ...
                    {!loading && (
                        ...
                        >
                            <Edit2 className="w-4 h-4 mr-2" /> {data?.rules ? "Edit Aturan" : "Buat Aturan"}
                        </Button>
                    )}
                </div>
            </div>
            ...
            {loading ? (
                <div className="rounded-xl border bg-card p-6 flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <Skeleton className="w-full h-40 bg-gray-200 dark:bg-gray-800" />
                    </div>
                </div>
            ) : (
                <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
                    <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="w-full lg:w-[350px] shrink-0 space-y-3">
                            <div
                                className="relative w-full aspect-[3/4] rounded-lg overflow-hidden border shadow-sm bg-muted group/image cursor-pointer"
                                onClick={() => data?.rules_image_url && setPreviewImage(data.rules_image_url)}
                            >
                                {data?.rules_image_url ? (
                                    <>
                                        <Image
                                            src={data.rules_image_url}
                                            alt="Rules Banner"
                                            fill
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-2">
                                        ...
                                        <p className="text-xs">Tidak ada gambar SK/Peraturan</p>
                                    </div>
                                )}
                            </div>
                            {data?.rules_image_url && (
                            ...
                            <div className="space-y-3">
                                ...
                                {data?.rules ? (
                                    ...
                                    <div className="text-center py-12 bg-muted/20 rounded-lg border border-dashed">
                                        <p className="text-muted-foreground">Belum ada tata tertib yang diatur.</p>
                                        <Button variant="link" onClick={() => setShowForm(true)} className="text-orange-600">
                                            Buat Sekarang
                                        </Button>
                                    </div>
                                )}

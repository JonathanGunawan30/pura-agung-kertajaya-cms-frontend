...
export function ProgramsList() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const result = await programsApi.get(entityType)
            setData(result)
        } catch (error) {
    ...
    if (showForm) {
        return <ProgramForm initialData={data} entityType={entityType} onClose={handleFormClose} />
    }
            ...
            <div className="rounded-xl border bg-card text-card-foreground shadow-sm overflow-hidden">
                <div className="p-5 bg-background/50 backdrop-blur-sm flex justify-between items-center">
                    <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span className="text-sm font-medium">Program Kerja <span className="capitalize">{entityType}</span></span>
                    </div>
                    ...
                    {!loading && (
                        ...
                        >
                            <Edit2 className="w-4 h-4 mr-2" /> {data?.work_program ? "Edit Program" : "Buat Program"}
                        </Button>
                    )}
                </div>
            </div>
            ...
            {loading ? (
                <div className="rounded-xl border bg-card p-6 flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1 space-y-4">
                        <Skeleton className="w-1/3 h-6 bg-gray-200 dark:bg-gray-800" />
                    ...
                    <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="w-full lg:w-[350px] shrink-0 space-y-3">
                            <div
                                ...
                                onClick={() => data?.work_program_image_url && setPreviewImage(data.work_program_image_url)}
                            >
                                {data?.work_program_image_url ? (
                                    <>
                                        <Image
                                            src={data.work_program_image_url}
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {data?.work_program ? (
                                ...
                                <div className="text-center py-12 bg-muted/20 rounded-lg border border-dashed">
                                    <p className="text-muted-foreground">Belum ada data program kerja yang diisi.</p>
